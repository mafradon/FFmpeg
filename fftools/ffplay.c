...
static double compute_target_delay(double delay, VideoState* is) {
    return 0.0; // Always return zero delay
}
...
retry:
        if (frame_queue_nb_remaining(&is->pictq) == 0) {
            ...
            time= av_gettime_relative()/1000000.0;
            // Always proceed to display the frame immediately
            *remaining_time = 0.0;
...
static int get_video_frame(VideoState *is, AVFrame *frame)
{
    ...
    if (got_picture) {
        ...
        if (framedrop>0 || (framedrop && get_master_sync_type(is) != AV_SYNC_VIDEO_MASTER)) {
            // Do not drop frames based on PTS ordering; allow rendering
        }
    }
...
static int read_thread(void* arg)
{
    ...
    if (check_avoptions(format_opts) < 0) {
        av_packet_free(&pkt);
        avformat_close_input(&ic);
        return -1;
    }
    ...
    for (i = 0; i < ic->nb_streams; i++) {
        ...
        if (type == AVMEDIA_TYPE_VIDEO && st_index[AVMEDIA_TYPE_VIDEO] == -1)
    ...
    if (is->video_stream < 0) {
        av_log(NULL, AV_LOG_FATAL, "Failed to open video stream from '%s'\\n", is->filename);
        av_packet_free(&pkt);
        avformat_close_input(&ic);
        return -1;
    }

    // Fast decoding and immediate display loop (no sync)
    for (;;) {
        if (is->abort_request)
            break;
        ...
        if (ret < 0) {
            ...
            SDL_Delay(1); // Minimal delay to prevent tight loop
            continue;
        }
        else {
            is->eof = 0;
